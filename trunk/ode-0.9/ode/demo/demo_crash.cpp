...
#define WALLMASS 0.1		// wall box mass
...
#define WALLHEIGHT 50		// height of wall
#define BOXGAP 0.9			// The gap between of eacho wall box
...
static int textures[10000];//set each obj's texture
static int boxes;
...
static dGeomID cannon_geom;
static dBodyID cannon_body;
...
static dReal cannon_angle_base= 0, cannon_angle=0,cannon_elevation=-1.2;

static bool keyboardPause;
...
static void start()
{
//	static float xyz[3] = {8.8548f,9.0843f,42.5900f};
//	static float hpr[3] = {-145.5f,2.5f,0.25f};
	
	static float xyz[3] = {-27.0377f,-2.990539f,42.2590f};
	static float hpr[3] = {-90.5f,-91.5f,0.25f};
	currentViewMode = 5;	
...
void makeCar(dReal x, dReal y, int &bodyI, int &jointI, int &boxI, int &sphereI)
{
	...
	sphereI	+= 4;
        ...
        dBodySetPosition (cannon_ball_body,x,y,CANNON_BALL_RADIUS);
        joint[jointI++] = dJointCreateHinge2 (world,0);
        dJointAttach (joint[jointI++],body[carbodyI],cannon_ball_body);
#endif
        // draw the cannon
        /*
        cannon_body = dBodyCreate (world);
        cannon_geom = dCreateBox(space,2,2,2);
        dBodySetMass (cannon_body,&m);
        dGeomSetBody (cannon_geom,cannon_body);
	dBodySetPosition(cannon_body,x,y,STARTZ);
        joint[jointI+1] = dJointCreateHinge2 (world,0);
        dJointAttach (joint[jointI+1],body[carbodyI],cannon_body);
*/
	//jointI++;
/*
        dsSetColor (1,1,0);
...
void viewMode(int m)
{
	...
	switch (m) {			
		case 0:
			{
				const dReal *cpo = dGeomGetPosition(sphere[0]);//get car position
				const dReal *cpoo = dGeomGetPosition(sphere[2]);//get wall position
				for(int i=0;i<3;i++)	{
					txyz[i]=cpoo[i];
					thpr[i]=cpo[i];
				}
...
#ifdef WALL
	bool offset = false;
	dReal rad = ((WALLWIDTH-(WBOXSIZE/2.0))/2)/2;
for(dReal numberWall=0;numberWall>-15;numberWall-= 5){
	for (dReal z = WBOXSIZE/2.0; z <= WALLHEIGHT; z+=WBOXSIZE)
	{
		offset = !offset;
		dReal y_max = (WALLWIDTH-z)/2;
//		dReal rad = y_max/2;
		dReal x_offset = 0;
		for (dReal y = (-WALLWIDTH+z)/2; y <= y_max+BOXGAP*y_max; y+=WBOXSIZE)
		{
			wall_bodies[wb] = dBodyCreate (world);
			textures[wb]=(offset? DS_WOOD: DS_NONE);
#ifdef CYCLEWALL			
			if(y<=rad)
				x_offset =((int)(sqrt(rad*rad-(rad-y)*(rad-y))*10000))/10000;//x^2=r^2+dy^2
			else
				x_offset = ((int)(sqrt(rad*rad-(y-rad)*(y-rad))*10000))/10000;//x^2=r^2+dy^2
#endif
			dBodySetPosition (wall_bodies[wb],-40+(x_offset)*WBOXSIZE+numberWall,y+y*BOXGAP*WBOXSIZE,z);
	...
	for (dReal x = 0.0; x < COLS*(LENGTH+RADIUS); x += LENGTH+RADIUS)
		for (dReal y = -((ROWS-1)*(WIDTH/2+RADIUS)); y <= ((ROWS-1)*(WIDTH/2+RADIUS)); y += WIDTH+RADIUS*2)
	dBodySetPosition (cannon_ball_body,x,y,CANNON_BALL_RADIUS+STARTZ+2);
#endif
#ifdef TRIMESH
...
static void command (int cmd)
{
	...
	case '3':
		currentViewMode = 3;
		viewMode(currentViewMode);//update current mode
		break;
	case '4':
		currentViewMode = 4;
		viewMode(currentViewMode);//update current mode
	...
	case 'x': {
		dMatrix3 R2,R3,R4;
		dRFromAxisAndAngle (R2,0,0,1,cannon_angle+cannon_angle_base);
		dRFromAxisAndAngle (R3,0,1,0,cannon_elevation);
		dMultiply0 (R4,R2,R3,3,3,3);
		//dReal cpos[3] = {CANNON_X,CANNON_Y,1};
		const dReal *cpo = dGeomGetPosition(box[0]);
		dReal cpos[3]={cpo[0],cpo[1],cpo[2]+1};
...
static void simLoop (int pause)
{
	...
	float cxyz[3],chpr[3];
	dsGetViewpoint(cxyz,chpr);
	dMessage (0,"xyz %e %e %e,hpr %e %e %e",cxyz[0],cxyz[1],cxyz[2],chpr[0],chpr[1],chpr[2]);
	...
	if (!keyboardPause) {
		...
		for (j = 0; j < joints; j++)
		{
			...
			cannon_angle_base = (turn-curturn);
	...
	dsSetColor (1,1,0);
	dMatrix3 R2,R3,R4;
	const dReal *carangle = dGeomGetRotation(box[0]);
	dRFromAxisAndAngle (R2,0,0,1,cannon_angle+cannon_angle_base);
	dRFromAxisAndAngle (R3,0,1,0,cannon_elevation);
	dMultiply0 (R4,R2,R3,3,3,3);
	//dReal cpos[3] = {car_x,car_y,1};
	const dReal *cpo = dGeomGetPosition(box[boxes-1]);//Get Car position
	dReal cpos[3]={cpo[0],cpo[1],cpo[2]+1};
	dReal csides[3] = {2,2,2};
	dsDrawBox (cpos,dGeomGetRotation(box[boxes-1]),csides);
	for (i=0; i<3; i++) cpos[i] += 1.5*R4[i*4+2];
	dsDrawCylinder (cpos,R4,6,0.5);
